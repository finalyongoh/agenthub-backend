...
|Community Post API|내부|Web UI|API 서버|HTTPS/REST `[추정]`|title, body, board_type, repo_id|post|401, 403|
|Comment API|내부|Web UI|API 서버|HTTPS/REST `[추정]`|post_id, body|comment|401, 403|
...
#### 6.2.7 커뮤니티 게시글 작성 API

- Method: `POST`
    
- Endpoint: `/api/community/posts`
    
- Description: 자유게시판 또는 레포별 토론방 게시글을 작성한다.
    
- Request:
    
    ```json
    {
      "boardType": "FREE|REPOSITORY",
...
|CommunityPost|자유게시판/레포 토론방 게시글|
|Comment|게시글 댓글|
|Reaction|게시글/댓글 반응|
...
erDiagram
    ...
    REPOSITORY ||--o{ COMMUNITY_POST : discussed
...
|community_posts|자유게시판/레포 토론방 게시글|post_id, user_id, repository_id, board_type, title, body, status|post_id|user_id, repository_id|repository_id nullable|
|comments|댓글|comment_id, post_id, user_id, body, status, created_at|comment_id|post_id, user_id|숨김/삭제 상태|
|reactions|반응|reaction_id, user_id, target_type, target_id, reaction_type|reaction_id|user_id|중복 제한 필요|
|reports|신고/제보|report_id, reporter_id, target_type, target_id, reason_type, description, status|report_id|reporter_id|분석 오류 제보 포함|
